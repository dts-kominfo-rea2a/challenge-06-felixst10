...
const bacaData = fnCallback => {
  ...
  fs.readFile(file1, {encoding: 'utf-8'}, (err, data) => {
    ...
    resultData.push(dataResult.message.split(" ")[1]);
  })
  ...
  fs.readFile(file2, {encoding: 'utf-8'}, (err, data) => {
    ...
    const dataResult = JSON.parse(data);
    resultData.push(dataResult[0].message.split(" ")[1]);
  })
  ...
  fs.readFile(file3, {encoding: 'utf-8'}, (err, data) => {
    ...
    const dataResult = JSON.parse(data);
    resultData.push(dataResult[0].data.message.split(" ")[1]);
    fnCallback(null, resultData);
  })
